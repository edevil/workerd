...
public:
  GPUUncapturedErrorEvent(kj::StringPtr type,
    ...
    KJ_DBG("GPUUncapturedErrorEvent cons", error_->getMessage());
  };

  static jsg::Ref<GPUUncapturedErrorEvent> constructor(kj::String type) = delete;
...
private:
  ...
  jsg::Ref<GPUError> getError() { return error_.addRef(); }
  void visitForGc(jsg::GcVisitor& visitor) { visitor.visit(error_); }
};
