...
GPUDevice::GPUDevice(jsg::Lock& js, wgpu::Device d)
  ...
  struct UncapturedErrorCallbackContext {
    GPUDevice* self;
    jsg::Lock& js;
  };
  auto uncaptured_ctx = new UncapturedErrorCallbackContext{this, js};

  device_.SetUncapturedErrorCallback(
      [](WGPUErrorType type, char const* message, void* userdata) {
        auto c = std::unique_ptr<UncapturedErrorCallbackContext>(
            static_cast<UncapturedErrorCallbackContext*>(userdata));
        if (c->self->getHandlerCount("uncapturederror") > 0) {
          ...
          c->self->dispatchEventImpl(c->js, kj::mv(ev));
          return;
        }
